...
noun_pos = ["NN", "NNS", "NNP"]
MR = ["mod", "amod", "s", "subj", "obj", "obj2", "nsubj"]
adj_pos = ["JJ", "JJS", "JJR"]


def opinion_for_aspect(reviews, aspect_dictionary, opinion_file):
    opinion_for_aspect = {}
    f = open(aspect_dictionary)
    g = open(opinion_file)
    opinions = [word.replace("\n", "") for word in g.readlines()]
                                    ...
                                    if j[0] in MR:
                                        if j[2] == (index + 1):
                                            ...
                                            if pos[relate_word_position - 1][1] in adj_pos:
                                                ...
                                                if o in opinions and o not in opinion_for_aspect[aspect]:
                                            ...
                                            if pos[relate_word_position - 1][1] in adj_pos:
                                                ...
                                                if o in opinions and o not in opinion_for_aspect[aspect]:
                                                    opinion_for_aspect[aspect].append(o)
        with open('process/opinion_for_aspect.json', 'w') as fp:
